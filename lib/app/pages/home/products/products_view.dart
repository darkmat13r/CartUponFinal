...
class _ProductsPageState extends ViewState<ProductsPage, ProductsController> {
  _ProductsPageState({String type})
      : super(ProductsController(
            type, DataCategoryRepository(), DataProductRepository()));
  ...
  get _categories => ControlledWidgetBuilder(
          builder: (BuildContext context, ProductsController controller) {
        return controller.categories != null && controller.categories.length > 0
            ? Container(
                height: 50,
  ...
  get _products => ControlledWidgetBuilder(
        ...
        double cardHeight = MediaQuery.of(context).size.width / 2;
        return StateView(
          controller.isLoading ? EmptyState.LOADING : EmptyState.CONTENT,
          GridView.builder(
            shrinkWrap: true,
